...
public class FileTailer {
	...
	private static Func1<Event, Observable<String>> reportNewLines(
			final File file, final AtomicLong currentPosition) {
		return new Func1<Event, Observable<String>>() {
			@Override
			public Observable<String> call(Event event) {
				if (file.length() > currentPosition.get()) {
					return trimEmpty(
							lines(createReader(file, currentPosition.get())))
					// as each line produced increment the current
					// position with its length plus one for the new
					// line separator
							.doOnNext(
									moveCurrentPositionByStringLengthPlusOne(currentPosition));
				} else
