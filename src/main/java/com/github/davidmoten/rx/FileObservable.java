...
public final class FileObservable {
	 ...
	 * @return
	 */
	public final static Observable<byte[]> tailFile(File file,
			long startPosition, long sampleTimeMs, int chunkSize) {
		Observable<Object> events = from(file,
	 ...
	 * @return
	 */
	public final static Observable<String> tailTextFile(File file,
			long startPosition, long sampleTimeMs, Charset charset) {
		return toLines(
				tailFile(file, startPosition, sampleTimeMs,
						DEFAULT_MAX_BYTES_PER_EMISSION), charset);
	}
	 ...
	 */
	public final static Observable<String> tailTextFile(File file,
			long startPosition, int chunkSize, Charset charset,
			Observable<?> events) {
	...
	public final static Observable<WatchEvent<?>> from(WatchService watchService) {
		return Observable.from(watchService).lift(
				new OperatorWatchServiceEvents());
	}
	 ...
	 * @return
	 */
	private final static Func1<WatchEvent<?>, Boolean> onlyRelatedTo(
			final File file) {
	...
	private static Observable<String> toLines(Observable<byte[]> bytes,
			Charset charset) {
	...
	private static Observable<Object> sampleModifyOrOverflowEventsOnly(
			Observable<?> events, final long sampleTimeMs) {
		return events.groupBy(isModifyOrOverflow()).flatMap(new Func1<GroupedObservable<Boolean,?>,Observable<?>>(){
	...
	private static Func1<Object, Boolean> isModifyOrOverflow() {
			...
			public Boolean call(Object event) {
				if (event instanceof WatchEvent) {
					...
					if (kind.equals(StandardWatchEventKinds.ENTRY_MODIFY.name())
							|| kind.equals(StandardWatchEventKinds.OVERFLOW
									.name())) {
						return true;
					} else
						return false;
	...
	public static class Builder {
		...
		public Observable<byte[]> tail() {
			return tailFile(file, startPosition, sampleTimeMs, chunkSize,
					source);
		}
